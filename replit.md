...
- `artifacts/admin` — DramaVerse Admin, a React + Vite panel for managing dramas, episodes, and global monetization/ad settings, fully wired to the live API. Episode list on the drama detail page (`DramaEdit.tsx`) supports bulk CSV import (parses the "集数,视频流文件路径" format — `第 N 集,<m3u8 url>` rows — upserting by episode number so re-imports update in place, no duplicates) plus multi-select checkboxes with a "Delete Selected" batch action.
